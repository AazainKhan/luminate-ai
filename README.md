...
5) **Streaming events flow back to the browser**  
   - Events: `trace-id`, `thinking` (decision trace), `sources` + `citations` (RAG hits), `reasoning-delta`, `text-delta`, `evaluation`, optional `error`, and final `finish` with `chatId`/`traceId`.  
