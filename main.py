...
from tools.db import log_message, save_rating
...
def ask(body: Ask):
    try:
        ...
        def _task_name(t: Any) -> str:
            """Normalize TaskType enum or str to a plain lowercase string."""
            if isinstance(t, str):
                return t.lower()
            # e.g. TaskType.EXPLAIN → "explain"
            if hasattr(t, "value"):
                return str(t.value).lower()
            return str(t).lower()

        def _task_label(t: Any) -> str:
        ...
        # Final combined assistant message shown in curl output
